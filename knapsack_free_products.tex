...
\medskip
Note that by exhaustively enumerating all pairs of vertices in $\Gamma$, this problem $\P$-time reduces to the following.
...
Second, we note that for a given group $G$, $\AGP(G)$ reduces both to $\SSP(G\ast F_2)$ and $\SSP(G\times F_2)$, where $F_2$ is a generated free group on two free generators.

\begin{proposition}\label{pr:agp_to_ssp_star}
Let $G$ be a finitely generated group, $F_2$ be the $2$-generated free group. Then $\AGP(G)$ $\P$-time reduces to $\AGP(G\ast F_2)$.
\end{proposition}
\begin{proof}
...
Let $G$ be a finitely generated group, $F_2$ be the $2$-generated free group. Then $\AGP(G)$ $\P$-time reduces to $\AGP(G\times F_2)$.
\end{proposition}
\begin{proof}
...
Based on the idea used in Proposition~\ref{pr:agp_to_ssp_cross}, we can slightly strengthen the statement in the case of $\BSMP$.

\begin{proposition}\label{pr:bsmp_to_ssp}
Let $G$ be a finitely generated group. Then $\BSMP(G)$ $\P$-time reduces to $\SSP(G\times \mathbb Z)$.
\end{proposition}
\begin{proof}
Picture!
\end{proof}

Either of the Propositions~\ref{pr:agp_to_ssp_cross} or~\ref{pr:bsmp_to_ssp} is enough to show the following result.
\begin{proposition}\label{pr:ssp_cross}
Let $F_2$ be the $2$-generated free group. $\SSP(F_2\times F_2\times F_2)$ is $\NP$-complete.
\end{proposition}
\begin{proof}
As we mentioned in the proof of Proposition~\ref{pr:agp_cross}, $\AGP(F_2\times F_2)$ is $\NP$-complete. By Proposition~\ref{pr:agp_to_ssp_cross}, the latter reduces to $\SSP((F_2\times F_2)\times F_2)$ (or by Proposition~\ref{pr:bsmp_to_ssp}, to $\SSP((F_2\times F_2)\times\mathbb Z)$). Therefore, the $\SSP(F_2\times F_2\times F_2)$ is $\NP$-complete.
\end{proof}
...
\end{corollary}
\begin{proof}
Combining Propositions~\ref{pr:agp_hyp} and~\ref{pr:ssp_cross}, we obtain at least one pair of the two $G=H=F_2$ or $G=F_2\times F_2$, $H=F_2$ delivers the statement.
\end{proof}
...
\begin{corollary}
It follows from Proposition~\ref{pr:bsmp_to_ssp} that $\SSP$ is $\NP$-complete in braid groups $B_n$, $n\ge 8$, matrix groups $SL(n,\mathbb Z)$, $n\ge 5$, partially commutative groups containing $F_2\times F_2\times \mathbb Z$.
\end{corollary}
