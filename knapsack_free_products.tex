...
Algorithmic problems in a direct product of groups can be dramatically more complex than in either factor, as is the case with the membership problem, first shown by Mikhailova~\cite{Mikhailova}. By contrast, the word problem in direct products easily reduces to that in the factors. {\li{We mention the conjugacy problem, so it'll be better to say something on that as well. Also, some references needed (\cite{Lyndon-Schupp:2001}, for example).}}
...
{\li{Why don't we say that these reductions are really different?}}
...
{\li{I don't think we need a susub below: you write about both products anyway.}}
...
{\li{I'm not sure I understand you correctly. If we write arbitrary long labels, an instance's size changes as much as it wants... So, what do you mean?}}
...
$$
{\li{(where $h \in G$, $m \in Z$ and $(h,m)$ meaning the corresponding label, etc. We need to write it, since we don't have any standard notations.}}
Let $\Gamma'$ {\li{$\Gamma' = (V', E')$}} be the minimal connected component of $\Gamma^\ast$ containing $(\alpha,0) \in V^\ast$
...
\end{corollary}
{\li{I would say, $\AGP(F_m\times \mathbb Z) \in \P$, where $F_m$ is the free group of rank $m$, $m < \infty$ instead.}}
...
Next, we produce in polynomial time $n$ freely independent elements $v_1,\ldots, v_j$ of the free group $F_2$ {\li{ $F_2= \langle x, y \rangle$}}, of which we think as labels of the corresponding vertices $V_1,\ldots, V_n$. For example, $v_j=x^jyx^j$, $j=1,\ldots,n$, suffice.
We claim that
...
\end{figure}
\end{proof}
{\li{This statement implies a natural question: what about Karp reduction?}}
...
\end{proof}
In the light of the above corollary, a natural question to ask is whether $\SSP(F_2\times \mathbb F_2)$ {\li{$\SSP(F_2\times F_2)$ }}is $\NP$-complete.
{\li{But we will answer another one, right?}}
...
\end{proof}
{\li{You cite this lemma below to get a normal form of $f^n$ for arbitrary $n$, in particular, $f^2$...}}
