...
In Section~\ref{sec:agp} we show that direct product does not preserve polynomial time subset sum problem. Thus, the subset problem occupies an interesting position, exhibiting behavior features of both word problem and membership problem; on the one hand, its decidability clearly carries immediately from factors to the direct product, while, on the other hand, its time complexity can increase dramatically (unless $\P=\NP$). % The now-classic construction in the latter work was recently exploited in~\cite{Miasnikov-Nikolaev-Ushakov:2014a} to show that a direct product does not preserve polynomial time bounded submonoid membership problem, unless $\P=\NP$. However, it is not a priori clear whether the subset sum problem should behave in a similar way.
